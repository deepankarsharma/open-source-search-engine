...
static void logError ( const char *reason, const char *xpath, size_t currentPos = 0 ) {
	if (currentPos > 0) {
		...
		if ( is_alpha_a( c ) ) {
			if ( !xmlXPath->m_hasNode && !xmlXPath->m_hasAttribute ) {
				...
				if ( nodeId == TAG_TEXTNODE ) {
					// invalid tag id
					logError( "invalid tag id", xpath, currentPos );
					return false;
				}
			...
			if (xmlXPath->m_hasPredicate) {
				...
				logError("multiple predicate found", xpath, currentPos);
				return false;
			} else {
			...
			if (xmlXPath->m_hasAttribute) {
				...
				logError("multiple attribute found", xpath, currentPos);
				return false;
			} else {
				++currentPos;
			...
			if ( xmlXPath->m_hasAttributeValue ) {
				...
				logError("multiple attribute value found", xpath, currentPos);
				return false;
				...
				if ( c == '\'' || c == '"' ) {
					...
					// no quotes found
					logError("attribute value without quote", xpath, currentPos);
					return false;
				}
			} else {
				// we should be in predicate & attribute to be able to support '=' sign
				logError( "unable to support '=' sign", xpath, currentPos );
				return false;
			}
		} else if ( c == '\'' || c == '"' ) {
			if ( xmlXPath->m_hasAttributeValue ) {
				if ( !xmlXPath->m_attributeValueLen ) {
					...
					logError("quote found with attribute value len set", xpath, currentPos);
					return false;
				}
			} else {
				// quotes found without attribute value
				logError("quote found without attribute value", xpath, currentPos);
				return false;
			}
		} else if ( c == '/' ) {
			if ( !xmlXPath->m_hasPredicate && xmlXPath->m_hasAttribute ) {
				logError("no predicate but has attribute and found child", xpath, currentPos);
				return false;
			}
			...
			if ( xmlXPath->m_hasAttribute && !xmlXPath->m_attributeLen ) {
				if ( !xmlXPath->m_hasAttributeValue ) {
					...
					logError( "attribute does not have length when attribute value is available", xpath );
					return false;
				}
			}
